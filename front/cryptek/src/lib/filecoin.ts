...
export async function initializeSynapse(): Promise<SynapseInstance> {
  ...
  try {
    ...
    throw new Error(
      `Error al inicializar Synapse: ${(error as Error).message}`
    );
  }
}
...
export async function prepareForStorage(dataSize: number) {
  ...
  try {
    ...
    console.log(
      `Balance USDFC en payments: ${ethers.formatUnits(balance, 18)}`
    );
    ...
    console.log(
      `Costo estimado: ${ethers.formatUnits(estimatedCost, 18)} USDFC/mes`
    );
...
export async function uploadToFilecoin(file: File): Promise<{
  ...
  try {
    ...
    console.log(
      `📁 Subiendo archivo de ${data.length} bytes a Filecoin Calibration...`
    );
    ...
    const storage = await synapse.createStorage({
      ...
      callbacks: {
        ...
        onProofSetCreationProgress: (status: any) => {
          ...
          console.log(
            `⏱️ [${elapsed}s] Minado: ${status.transactionMined}, Activo: ${status.proofSetLive}`
          );
        },
      },
    });
...
export async function downloadFromFilecoin(commP: string): Promise<Uint8Array> {
  ...
  try {
    ...
    throw new Error(
      `Error al descargar de Filecoin: ${(error as Error).message}`
    );
  }
}
