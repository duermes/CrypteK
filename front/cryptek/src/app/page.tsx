...
export default function Home() {
  ...
  const handleUploadToFilecoin = async () => {
    ...
    try {      
      ...
      alert(`✅ Archivo subido a Filecoin!\nCommP: ${result.commP}\nTamaño: ${result.size} bytes`);
    } catch (error) {
    ...
    <main className="p-6 space-y-6 max-w-2xl mx-auto">
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        ...
        {!isConnected ? (
          ...
          <div className="space-y-1">
            <div className="text-green-600">✅ Conectado: {address?.slice(0, 6)}...{address?.slice(-4)}</div>
      ...
      {storageInfo && (
        <div className="bg-purple-50 p-4 rounded-lg">
          ...
          <div className="text-sm text-purple-700 space-y-1">
            <div>Red: <strong>{storageInfo.network}</strong></div>
            <div>Época actual: <strong>{storageInfo.currentEpoch}</strong></div>
            <div>Balance USDFC (Payments): <strong>{storageInfo.paymentsBalance}</strong></div>
            <div>Balance USDFC (Wallet): <strong>{storageInfo.walletBalance}</strong></div>
          </div>
        </div>
      )}
      ...
      <div className="border rounded-lg p-4">
        ...
        <div className="space-y-3">
          ...
          >
            {isPreparingStorage ? "Preparando..." : 
             isUploading ? "Subiendo a Filecoin..." : 
             "📁 Subir a Filecoin"}
          </button>
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    const blob = new Blob([new Uint8Array(data)], { type: file?.type || 'application/octet-stream' }); 
      ...
      <div className="bg-blue-50 p-4 rounded-lg">
        ...
        <ol className="text-sm text-blue-700 space-y-1">
          <li>1. 📁 Media se almacena en <strong>Filecoin</strong> (descentralizado)</li>
          <li>2. 📝 Mensajes se guardan en <strong>Lisk</strong> (rápido y barato)</li>
